...
Vediamo infatti che l'uscita del discriminatore è caratterizzata da un singolo elemento dotato di funzione di trasferimento sigmoide, la quale nel
...
\begin{figure}[H]
    ...
    \begin{tabular}{cc}
        ...
        \subfloat[Saturazione per $\mathbf{y \rightarrow 0}$]{
            ...
            \centering
            \includegraphics[width=0.3\textwidth]{imgs/graphs/sigmoid_grad_vanish_0.png}
        }  &
        \subfloat[Saturazione per $\mathbf{y \rightarrow 1}$]{
            \label{fig:sig_grad_1}
            \centering
            \includegraphics[width=0.3\textwidth]{imgs/graphs/sigmoid_grad_vanish_1.png}
        }  \\
    \end{tabular}
\end{figure}

Per visualizzare meglio questo fenomeno possiamo utilizzare un grafico, ipotizzando di visualizzare i dati del training set su una sola dimensione,
...
\end{equation}
\subsubsection{Il mode collapse}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{imgs/mode_collapse.png}
...
apprende una rappresentazione dei dati nello spazio latente in maniera profonda e relativa alle caratteristiche che essi presentano.

Vediamo un esempio tratto dallo stesso articolo, creato attraverso un modello addestrato su un dataset di facce umane.
Nell'esempio vengono presi 3 vengono presi 3 vettori per ognuna di 3 aree di Z: "donna sorridente", "donna seria", "uomo serio" e viene fatta la media 
di questi vettori, ottenendo ancora un vettore che rappresenta le stesse caratteristiche, ciò indica che queste tre aree sembrano avere una sorta di 
...
non linearità, per cui in generale è difficile effettuare operazioni di questo tipo con un buon controllo del risultato.
    ...
    \end{figure}
Si noti che le medesime operazioni fatte a livello di pixel danno risultati privi di senso.
