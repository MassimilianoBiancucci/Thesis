...
A questo punto possiamo introdurre i prodotti parziali del gradiente per determinare il gradiente della funzione di errore as un determinato layer $l$:
...
In tal modo possiamo calcolare i prodotti parziali del gradiente per ogni layer, partendo dallo strato di output all'indietro, minimizzando così il numero di moltiplicazioni
...
qualunque altra funzione continua. Nel caso particolare delle reti neurali vi è una grande quantità di varianti, le quali
hanno una diversa variante del teorema di approssimazione universale, che ne dimostra le capacità di approssimazione.
...
dimostrò che una combinazione lineare di composizione di una predeterminata funzione univariata e un insieme di funzioni affini può approssimare 
qualsiasi funzione continua di n variabili reali con supporto in un ipercubo unitario.
%TODO: verifica se è corretto e se va aggiunto altro

Sempre nel 1989 Kurt Hornik et al in \textit{Multilayer feedforward networks are universal approximators} \cite{hornik1989multilayer} dimostrano che
%TODO: continua...
...
in un sistema di riconoscimento di numeri scritti a mano in giapponese. Sempre a Fukushima si deve 
anche l'introduzione della funzione di trasferimento \textit{RELU}, precedentemente discussa.
